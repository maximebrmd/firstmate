...
# loop below accepts a pane cwd that differs from the project dir, but shell-init
# noise (e.g. a transient ~/.oh-my-zsh cwd during zsh startup - itself a git repo,
# but a DIFFERENT repository) can appear before `treehouse get` lands. Comparing
# canonical git common dirs distinguishes the real lease from such noise no matter
# where treehouse.toml places the pool root, and resolving every path with pwd -P
# keeps a symlinked $HOME from breaking the match. The isolation guard after the
...
if [ "$KIND" != secondmate ] && [ "$BACKEND" != orca ]; then
  ...
  #
  # is_project_worktree is the other half: a read that merely differs from the
  # project can still be shell-init noise (a transient ~/.oh-my-zsh cwd, itself a
